...
def load_json_file(file_path):
    ...
    if not raw_data.strip():  # Skip if the file is empty
        print(f"Skipped empty file: {file_path}")
        return None

    # Step 3: Open with detected encoding and load JSON
    try:
        with open(file_path, "r", encoding=detected_encoding) as f:
            return json.load(f)
    except json.JSONDecodeError as e:
        print(f"Skipped invalid JSON file: {file_path} - Error: {e}")
        return None
...
for file_path in json_files:
    ...
    if data is None:
        continue  # Skip to the next file if invalid or empty

    # Process valid JSON data below

    # Extract session info
    session_date = file_path.split(os.sep)[-1].split('_')[0]  # Assuming date is part of the filename

    # Extract session info
#    session_date = file.split('_')[0]  # Assuming date is part of the filename, like "241120_184021_FP.json"
